...
say "1/6 ищу настройки TURN"
TS=""
for c in /etc/turnserver.conf /etc/coturn/turnserver.conf /etc/default/coturn /etc/turnserver/turnserver.conf; do
  [ -f "$c" ] && grep -qE "^(realm|listening-port|cert)" "$c" 2>/dev/null && TS="$c" && break
done
if [ -z "$TS" ]; then
  # ищем по всей системе — вдруг лежит нестандартно
  TS="$(grep -rlE "^realm=" /etc 2>/dev/null | grep -i turn | head -1 || true)"
fi
if [ -z "$TS" ]; then
  echo "!!! не нашёл конфиг TURN. Вот что есть в системе:"
  echo "--- службы ---"; systemctl list-units --all 2>/dev/null | grep -i turn | sed 's/^/  /'
  echo "--- пакеты ---"; dpkg -l 2>/dev/null | grep -iE "coturn|turnserver" | sed 's/^/  /'
  echo "--- файлы ---";  ls -la /etc/turnserver.conf /etc/coturn/ 2>/dev/null | sed 's/^/  /'
  echo "--- кто слушает 3478 ---"; ss -lnup 2>/dev/null | grep 3478 | sed 's/^/  /'
  echo ""
  echo "Пришли этот вывод мне — по нему пойму, где настройки."
  exit 1
fi
echo "конфиг TURN: $TS"
